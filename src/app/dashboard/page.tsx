...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch('https://zero-to-one-4.onrender.com/trainings-available/', {
          method: "GET",
          headers: {
  ...
  const handleTrainingClick = (trainingId: string | undefined) => {
    console.log('changing to dashboard/trainingId')
    router.push(`/dashboard/${trainingId}`);
    console.log('changed to dashboard/trainingId')
  };
    ...
    <div className="flex items-center">
      <div className="text-sm">
        <button className="border-2 border-green-600 rounded-full px-8 py-2 inline-block font-semibold hover:bg-green-600
                     hover:text-white text-green-900" onClick={() => handleTrainingClick(training.id)}>Book Training</button>
