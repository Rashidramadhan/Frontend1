  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Fetched topics:", data); // Log the fetched topics
        setTopics(data);
      } catch (error) {
                ...
                <div className="grid grid-cols-1 gap-6">
                  <label className="block">
                    ...
                    <input
                      type="text"
                      name="phone_number"
                      className="mt-1 block w-full rounded-md border-transparent bg-gray-100 focus:border-gray-500 focus:bg-white focus:ring-0 px-2"
                      placeholder=""
                  ...
                  <label className="block">
                    ...
                    >
                      <option value="beginner" className="font-bold">Beginner</option>
                      <option value="intermediate" className="font-bold">Intermediate</option>
                      <option value="experienced" className="font-bold">Experienced</option>
                    </select>
                  </label>
                    ...
                    <nav className="flex min-w-[100px] flex-row gap-1 p-2">
                      {topics.length > 0 ? (
                        topics.map((topic: Topic) => {
                          console.log("Rendering topic:", topic); // Log each topic during rendering
                          return (
                            <div
                              key={topic.id}
                              role="button"
                  ...
                  <button
                    ...
                    disabled={isLoading}
                  >
                    {isLoading ? <span>Creating Profile...</span> : <span>Save Profile</span>}
                  </button>
                </div>
              </div>
            </div>
